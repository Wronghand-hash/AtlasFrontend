  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <a
            :href="`https://auth.atlasacademy.ir/registrations/file/${props.request.colleagesResume[0].id}`"
          >
...
const downloadResume = async () => {
  ...
  const { data } = await $fetch(
    `https://auth.atlasacademy.ir/registrations/file/${props.request.colleagesResume[0].id}`,
    {
      headers: {
        "Content-Type":
