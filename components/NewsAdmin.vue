...
const removeNewsImage = async function () {
  ...
  if (props.news.NewsImages.length) {
    await $fetch(
      `https://auth.atlasacademy.ir/management/newsimageremove/${props.news.NewsImages[0].id}`,
      {
        method: "POST",
        headers: {
...
const removeNews = async function () {
  await $fetch(`https://auth.atlasacademy.ir/management/newsremove/${props.news.id}`, {
    method: "POST",
    headers: {
      "Content-Type": "application/x-www-form-urlencoded",
    },
