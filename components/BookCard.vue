    ...
    >
      ...
      <button>
        <a
          ...
          :href="`https://auth.atlasacademy.ir/books/file/${book.id}`"
        >
...
const dowloadBook = async () => {
  ...
  const { data } = await $fetch(
    `https://auth.atlasacademy.ir/books/file/${props.book.id}`,
    {
      headers: {
        "Content-Type":
...
const getBooksImage = async () => {
  ...
  const { data } = await $fetch(
    `https://auth.atlasacademy.ir/books/image/${props.book.BooksImages[0].id}`,
    {
      headers: {},
      withCredentials: true,
      credentials: "include",
